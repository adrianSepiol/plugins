...
import type { BoundingBox, ResizeHandleId } from '../types/editor-types';
...
export function handlePosition(bbox: BoundingBox, h: ResizeHandleId): { x: number; y: number } {
  const [tx, ty] = HANDLE_POSITIONS[h];
  return {
    x: bbox.minX + (bbox.maxX - bbox.minX) * tx,
    y: bbox.minY + (bbox.maxY - bbox.minY) * ty,
  };
}
